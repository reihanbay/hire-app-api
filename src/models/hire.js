...
module.exports = {
  ...
  getHireModel: (searchKey, searchValue, limit, offset) => {
    return new Promise((resolve, reject) => {
      db.query(`SELECT hire.*, recruiter.nameRecruiter AS recruiter FROM hire INNER JOIN projects ON hire.idProject = projects.idProject INNER JOIN recruiter ON projects.idRecruiter = recruiter.idRecruiter WHERE ${searchKey} LIKE '%${searchValue}%' GROUP BY idHire LIMIT ${limit} OFFSET ${offset} `, (err, result, _fields) => {
        if (!err) {
          resolve(result)
        } else {
          reject(new Error(err))
  ...
  getHireByIdModel: (id) => {
    return new Promise((resolve, reject) => {
      db.query(`SELECT hire.* , recruiter.nameRecruiter FROM hire INNER JOIN projects ON hire.idProject = projects.idProject INNER JOIN recruiter ON projects.idRecruiter = recruiter.idRecruiter WHERE hire.idHire = ${id} GROUP BY idHire`, (err, result, _field) => {
        if (!err) {
          resolve(result)
        } else {
          reject(new Error(err))
